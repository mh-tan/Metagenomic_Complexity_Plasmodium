...
# ENV: UNZIP FILES
```{bash}
gunzip -c1 input_files/DBS_PilotS1to8_WB_S8MRS.id96.DBLaTags_cleaned_renamed_centroids.fasta.gz > input_files/DBS_PilotS1to8_WB_S8MRS.id96.DBLaTags_cleaned_renamed_centroids.fasta
gunzip -c1 input_files/DBS_PilotS1to8_WB_S8MRS.id96.DBLaTags_cleaned_renamed_otuTable_binary.txt.gz > input_files/DBS_PilotS1to8_WB_S8MRS.id96.DBLaTags_cleaned_renamed_otuTable_binary.txt
...
for (plotpid in setpid) {
  ...
  ori_S8_demography_file <- read.table("input_files/Ghana_MRS_Survey8_Epi_SpPCR_Var_281123_FINAL.txt", header=TRUE, sep="\t", check.names=FALSE)
  
  ## Survey 8 DBS-MOI data - to get correct isolates
  ori_S8_DBS_MOI_file <- read.csv("input_files/Ghana_MRS_Survey8_Epi_MOI_131223.txt", header=TRUE, sep="\t", check.names=FALSE)
  
  ## S8 WB-species data
  ori_species_file <- read.table(paste0("input_files/Ghana_MRS_Survey8_DBS_speciesPCR.txt"), header=TRUE, sep="\t", check.names=FALSE) %>%
    select(-c(starts_with("DBS_")))
  
  ## S8 WB-species MOI data
  ori_Pm_MOI_file <- read.csv(paste0("input_files/Ghana_MRS_Survey8_WB_Pm_MOI.csv"), header=TRUE)
  ori_Poc_MOI_file <- read.csv(paste0("input_files/Ghana_MRS_Survey8_WB_Poc_MOI.csv"), header=TRUE)
  ori_Pow_MOI_file <- read.csv(paste0("input_files/Ghana_MRS_Survey8_WB_Pow_MOI.csv"), header=TRUE)
  
  ## S8 WB-plasma-hrp2 data
  ori_hrp2_file <- read.table(paste0("input_files/Ghana_MRS_Survey8_ELISA_HRP2.txt"), header=TRUE, sep="\t", check.names=FALSE)
  
  ## GADM Map data
  ori_MAPs_subnational_file <- read.csv("input_files/MAP_Subnational_Unit-data.csv", header=TRUE, check.names=FALSE, comment.char="")
  ori_MAPs_bongo_file <- read.csv("input_files/Pixel-level-data_BongoGhana.csv",
  ...
  saveRDS(ori_S8_demography_file, paste0("Z01.MAIN.out_RDS/0", PART, ".id", plotpid, ".S8MRS_DBS_demography.", file_prefix, ".rds"))
  ...
  saveRDS(ori_MAPs_bongo_file, paste0("Z01.MAIN.out_RDS/0", PART, ".id", plotpid, ".MAPs_Bongo_data.", file_prefix, ".rds"))
    
  ##### Delete variables
  rm(ori_otutable_file, ori_domain_file, ori_isolates_file, ori_fasta_file, ori_S8_demography_file, ori_hrp2_file, ori_S8_DBS_MOI_file, ori_species_file, ori_Pm_MOI_file, ori_Poc_MOI_file, ori_Pow_MOI_file, ori_MAPs_subnational_file, PART)
}
```
...
for (plotpid in setpid) {
  ...
  plot_tile_MOI_WB <- file_isolates_MOI %>%
    filter(Species == "Pf" & WBvol %in% WBvol_list) %>%
    mutate(AgeGrp=factor(AgeGrp, levels=AgeGroups_list)) %>%
  ...
  file_isolates_MOI_age_label <- file_isolates_MOI %>%
    select(Isolate, AgeGrp, label_long) %>%
    unique()
  
  file_isolates_MOI_DBS <- file_isolates_MOI %>%
    filter(Species == "Pf") %>%
    select(Isolate, WBvol, MOI) %>%
    droplevels() %>%
    complete(Isolate, WBvol) %>%
    filter(WBvol == "DBS") %>%
    left_join(., file_isolates_MOI_age_label, by=c("Isolate"))
...
for (plotpid in setpid) {
  ...
  for (pw_metric in c("Pf-MOI")) {
    ...
    pw_metric_name <- case_when(pw_metric == "Pf-MOI" ~ "MOI")
    ...
    ## Plot
    max_value <- max(c(file_isolates_field_metric_pivot$volS_value, file_isolates_field_metric_pivot$volB_value))
    ...
    for (row in 1:nrow(WBvol_pairs_df)) {
      ...
      ## Filter data for specific pairings
      file_isolates_field_metric_pivot_tmp <- file_isolates_field_metric_pivot %>%
        filter(label == pw_pair_label) %>%
        select(-Isolate) %>%
        group_by(metric, volS_value, volB_value, volS_name, volB_name, label_long) %>%
        summarise(pointsize=n()) %>%
        ungroup() 
      
      ## Filter CCC df for specific pairings
      CCC_df_metric_tmp <- CCC_df_metric %>% filter(label == pw_pair_label)
        
      plot_scatter <- file_isolates_field_metric_pivot_tmp %>%
        ggplot(., aes(x=volS_value, y=volB_value)) +
        geom_point(aes(size=pointsize, alpha=0.8), fill="grey30", color="black", shape=21) +
        geom_abline(intercept=0, slope=1, linewidth=0.5, linetype="dashed", color="black") +
        geom_abline(data=CCC_df_metric_tmp, aes(intercept=alpha_val, slope=beta_val), color="black") +
        geom_text(data=CCC_df_metric_tmp, aes(x=min(as.numeric(V1)), y=max(as.numeric(V2)), label=label_ccc), hjust=0) +
        theme_bw() +
        scale_x_continuous(name=bquote(.(pw_metric)[.(pw_volS)]), breaks=scales::pretty_breaks(),
                           limits=c(0,max_value)) +
        scale_y_continuous(name=bquote(.(pw_metric)[.(pw_volB)]), breaks=scales::pretty_breaks(),
                           limits=c(0,max_value)) +
        ...
        theme(legend.position="top",
              ...
              panel.grid=element_blank()) +
        facet_wrap(.~label_long) +
        theme(strip.text=element_text(size=12))
      
      ## Save individual plots to list
      plot_list[[pw_pair_label]] <- plot_scatter
    }
    
    plot_scatter_combined <- ggarrange(plotlist=plot_list, nrow=2, ncol=3, common.legend=TRUE)
    
    ## Save combined plots to list
    plot_list_combined[[pw_metric]] <- plot_scatter_combined
  }
...
for (plotpid in setpid) {
  ...
  fig_PTS_scatter_colParDen <- isolates_PTS_MOI %>%
    ...
    scale_x_continuous(name=expression(PTS[L]), limits=c(0,1)) +
    scale_y_continuous(name=expression(PTS[S]), limits=c(0,1)) +
    viridis::scale_fill_viridis(name="Parasite density",
                                trans = scales::log10_trans(),
  ...
  ## Scatter: Colored by Parasite density (parasite density by bins)
  isolates_PTS_MOI_bin <- isolates_PTS_MOI %>%
    mutate(ParasiteuL_bin = case_when(ParasiteuL == 0 ~ "0",
                                      ParasiteuL >= 40 & ParasiteuL < 1000 ~ "40-999", ## Microscopy limit min at 40 parasites / uL
                                      ParasiteuL >= 1000 & ParasiteuL < 10000 ~ "1,000-9,999",
                                      ParasiteuL >= 10000 ~ "≥10,000")) %>%
    mutate(ParasiteuL_bin = factor(ParasiteuL_bin, levels=c("0", "40-999", "1,000-9,999", "≥10,000")))
    
    fig_PTS_scatter_colParDen_bin <- isolates_PTS_MOI_bin %>%
      ggplot(., aes(x=PTS_BtoA, y=PTS_AtoB, fill=ParasiteuL)) +
      geom_abline(intercept=0, slope=1, color="grey", linetype="dashed") +
      geom_point(color="black", shape=21, size=2) +
      theme_bw() +
      scale_x_continuous(name=expression(PTS[L]), limits=c(0,1)) +
      scale_y_continuous(name=expression(PTS[S]), limits=c(0,1)) +
      viridis::scale_fill_viridis(name="Parasite\ndensity",
                                  trans = scales::log10_trans(),
                                  breaks = scales::trans_breaks("log10", function(x) 10^x) (c(1, 10^5)),
                                  labels = scales::trans_format("log10", scales::math_format(10^.x))) +
      theme(legend.position="right",
      ...
      facet_wrap(ParasiteuL_bin ~ label_long, nrow=4) +
      theme(strip.text=element_text(size=12))
  ...
  ggsave(paste0("Z02.FIELD.out_RESULTS/0", PART, ".id", plotpid, ".FIELD.PTS_ColorByFoundMinCode.SCATTER.tiff"),
         ...
         device="tiff", dpi=300,
         width=8, height=4)
  # ggsave(paste0("Z02.FIELD.out_RESULTS/0", PART, ".id", plotpid, ".FIELD.PTS_ColorByParDensityBin.SCATTER.tiff"),
  #        fig_PTS_scatter_colParDen_bin,
  #        device="tiff", dpi=300,
  #        width=10, height=10)
...
for (plotpid in setpid) {
  ...
  for (ups in ups_list) {
    ...
    file_otutable_ups_t <- file_otutable_ups_t %>%
      ...
      select(-c("Samples", "Isolate", "MID":"village_ML", "age":"found_min20_code")) %>%
...
for (plotpid in setpid) {
  ...
  ## Delete variables
  rm(file_isolates_field, dt_field, dt_dbs, dt_repeat, file_otutable, file_fasta, filtered_fasta)
}
```
...
file_MAPs_Bongo_file <- readRDS(paste0("Z01.MAIN.out_RDS/0", PART_prev, ".id", plotpid, ".MAPs_Bongo_data.", file_prefix, ".rds"))
...
# Add data for Pf in Bongo
bongo_value <- file_MAPs_Bongo_file %>%
  filter(Year=="2020") %>%
  filter(Metric=="Infection Prevalence") %>%
  select(Value)

# Replace Upper East prevalence with Bongo prevalence
MAPs_data_subset <- MAPs_data_subset %>%
  mutate(Value=ifelse(`National Unit`=="Ghana" & Name=="Upper East" & Species == "Pf", bongo_value$Value, Value))

# Add NA for Pm and Po due to absence of MAPs data
MAPs_data_subset <- MAPs_data_subset %>%
  filter(Name %in% c("Bongo", "Upper East") & Species=="Pf") %>%
...
for (plotpid in setpid) {
  ...
  S8_DBS_data <- read.table("input_files/Ghana_MRS_Survey8_Epi_MOI_131223.txt", header=TRUE, sep="\t", check.names=FALSE) %>%
    filter(age >= 6)
